...
//TESTCODE:BEGIN
public class VersioningBurlapSerializerTest extends AbstractTestBase {

    @Override
    protected final VersioningSerializer createVersioningSerializer(final ClassesHistory history) {
        return new VersioningBurlapSerializer(history);
    }

    @Test
    public final void testDeserializeBurlapV1() throws IOException, DeserializationException {
        final File file = new File(DATA_DIR, "AddressV1-burlap.xml");
    ...
    @Test
    public final void testDeserializeBurlapV2() throws IOException, DeserializationException {
        final File file = new File(DATA_DIR, "AddressV2-burlap.xml");
        final Address result = read(file);
        assertThat(result.getId()).isEqualTo(100000);
        assertThat(result.getFirstName()).isEqualTo("John");
        assertThat(result.getLastName()).isEqualTo("Doe");
    ...
    @Test
    public final void testDeserializeBurlapV3() throws IOException, DeserializationException {
        final File file = new File(DATA_DIR, "AddressV3-burlap.xml");
        final Address result = read(file);
    ...
    public static void main(final String[] args) throws IOException {
        ...
        final File file = new File(DATA_DIR, "AddressV3-burlap.xml");
        write(file, address, new VersioningBurlapSerializer(readHistory()));
