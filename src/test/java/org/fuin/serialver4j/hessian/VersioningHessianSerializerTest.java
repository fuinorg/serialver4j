...
//TESTCODE:BEGIN
public class VersioningHessianSerializerTest extends AbstractTestBase {

    @Override
    protected final VersioningSerializer createVersioningSerializer(final ClassesHistory history) {
        return new VersioningHessianSerializer(history);
    }

    @Test
    public final void testDeserializeHessianV1() throws IOException, DeserializationException {
        final File file = new File(DATA_DIR, "AddressV1-hessian.bin");
    ...
    @Test
    public final void testDeserializeHessianV2() throws IOException, DeserializationException {
        final File file = new File(DATA_DIR, "AddressV2-hessian.bin");
        final Address result = read(file);
        assertThat(result.getId()).isEqualTo(100000);
        assertThat(result.getFirstName()).isEqualTo("John");
        assertThat(result.getLastName()).isEqualTo("Doe");
    ...
    @Test
    public final void testDeserializeHessianV3() throws IOException, DeserializationException {
        final File file = new File(DATA_DIR, "AddressV3-hessian.bin");
        final Address result = read(file);
    ...
    public static void main(final String[] args) throws IOException {
        ...
        final File file = new File(DATA_DIR, "AddressV2-hessian.bin");
        write(file, address, new VersioningHessianSerializer(readHistory()));
